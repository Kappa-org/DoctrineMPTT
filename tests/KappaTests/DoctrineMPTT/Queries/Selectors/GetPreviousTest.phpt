 ...
 */
class GetPreviousTest extends ORMTestCase
{
	...
	public function testGetParents()
	{
		...
		Assert::equal(2, $this->repository->fetchOne(new GetPrevious($this->configurator, $actual))->getId());
	}
}

Environment::lock("database", dirname(TEMP_DIR));

\run(new GetPreviousTest());
